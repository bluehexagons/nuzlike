...
(() => {
            ...
            for (let i = 0; i < this.sprites.length; i++) {
                ...
                if (args.length === 0) {
                    ...
                    for (let n = 0; n < l; n++, j++) {
                        ...
                        ctx.imageSmoothingEnabled = false;
                        ctx.drawImage(this.image, (j % this.width) * this.cellSize, (j / this.width | 0) * this.cellSize, this.cellSize, this.cellSize, 0, 0, tileSize, tileSize);
                        let d = 333;
                        if (frames.length > 0) {
                            d = frames[n];
                        }
    ...
    let drawSprite = (t, ctx, sprite) => {
        for (let i = 0; i < sprite.length; i++) {
            let s = sprite[i];
            if (s.animated) {
                let f = s.frames[s.frame];
                s.animate(t);
            }
    ...
    let mergeCache = new Map();
    let mergeSprites = (sprites) => {
    ...
    class Cell {
        constructor(bg, fg) {
            this.bgAnim = null;
            this.olSprites = []; // overlay sprites.. they should point to bg if animated, only drawn if fg or sprite over
            this.updated = true;
            this.dirty = true;
            this.bg = null;
            this.img = null;
